...
console.log(document.documentElement); // html 문서 내의 모든 element(요소)들을 가지고 ...
console.log(document.head); // head 태그와 내부 element들
console.log(document.body); // body 태그와 내부 element들
console.log(document.URL); // http, www, ... 전부 포함
console.log(document.domain); // ip 주소나 우리가 아는 도메인
...
console.log(document.querySelectorAll('div.pink')); // 추가
...
// 유사 배열
// length 속성을 가짐 
// [i]로 접근 가능 -> 인데스로 접근 가능 
// 표준 객체 메서드 사용 불가 (map)
// 하지만 forEach는 사용 가능 

// for of 문으로 pink 클래스 모두 출력하기 
console.log('for of문으로 pink class 모두 출력하기')
